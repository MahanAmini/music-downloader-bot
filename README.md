...
- 📜 دریافت لیریکس از [Genius API](https://genius.com/developers) با استفاده از کتابخونه‌ی `lyricsgenius`
- 🔁 در صورت پیدا نشدن نتیجه، استفاده از یه منبع **fallback** برای افزایش شانس پیدا کردن لیریکس
...
### 🔜 در آینده (بعد از MVP)
- جستجوی فازی / تشخیص غلط املایی
- کش کردن نتایج جستجوهای قبلی
- نمایش چند نتیجه برای انتخاب کاربر
...
| منبع اصلی لیریکس | [lyricsgenius](https://github.com/johnwmillr/LyricsGenius) |
| منبع fallback | یه API جایگزین (مثل lyrics.ovh) |
...
│   ├── start.py
│   ├── search.py             # هندل کردن جستجوی آهنگ
│   └── language.py           # تغییر زبان
├── services/
│   └── lyrics_service.py    # منطق گرفتن لیریکس (genius + fallback)
...
3. ربات لیریکس رو برات برمی‌گردونه — اگه طولانی باشه، در چند پیام ارسال می‌شه
4. با دستور تغییر زبان می‌تونی بین فارسی/انگلیسی جابه‌جا شی

---

## ⚖️ نکته‌ی کپی‌رایت

متن آهنگ‌ها معمولاً کپی‌رایت دارن. این ربات فقط از طریق APIهایی که مجوز نمایش این محتوا رو دارن (مثل Genius) لیریکس رو دریافت می‌کنه و هیچ ذخیره‌سازی انبوه یا توزیع مجدد لیریکس بیرون از context ربات انجام نمی‌ده.
